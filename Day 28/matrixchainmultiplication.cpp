...
int matrixMultiplication(vector<int> &arr, int N) {
    ...
    */
    ...
    /* State: dp[i][j] = Minimum number of scalar multiplications needed to compute the matrix A[i]A[i+1]...A[j] = A[i..j]
       where dimension of A[i] is arr[i-1] x arr[i]
    */
    ...
    for (int l = 2; l < N; l++) {
        for (int i = 1; i < N - l + 1; i++) {
            ...
            for (int k = i; k <= j - 1; k++) {
                ...
                // If temporary answer 'temp' is less than actual answer then update actual answer i.e dp[i][j]
